...
export default function ManageData() {
  ...
  const availableForP2 = availablePlayers.filter(
    (p) => String(p.id) !== teamForm.p1
  );
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-12">
      <div className="text-center space-y-4">
        ...
        <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
          Manage your roster. Add players first, then form teams to compete in
          the bracket.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
          ...
          <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-muted-foreground">
                Available {availableCount} / Total {totalPlayers} (Used{" "}
                {usedCount})
              </span>
            </div>
        ...
        <section className="bg-card rounded-2xl p-6 border border-border shadow-sm">
          ...
          <form
            ...
            className="space-y-4 mb-8 bg-secondary/20 p-5 rounded-xl border border-border/50"
          >
            <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select
                  value={teamForm.p1}
                  onChange={(e) =>
                    setTeamForm((p) => ({ ...p, p1: e.target.value }))
                  }
                  className="w-full px-4 py-2 rounded-lg bg-background border border-border focus:border-accent focus:outline-none focus:ring-2 focus:ring-accent/10"
                >
              ...
              <div>
                ...
                <select
                  value={teamForm.p2}
                  onChange={(e) =>
                    setTeamForm((p) => ({ ...p, p2: e.target.value }))
                  }
                  className="w-full px-4 py-2 rounded-lg bg-background border border-border focus:border-accent focus:outline-none focus:ring-2 focus:ring-accent/10"
                >
                  <option value="">Select...</option>
